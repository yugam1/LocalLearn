...
│   ├── api/           Contracts.java — the 7 interfaces you implement
│   ├── exercises/     Exercises.java  ← YOUR WORK GOES HERE (all 7 are broken)
...
All seven start broken. Each one is a bug you watched happen in a demo.
...
This is the **foundations block**. Still to build:

- Executors & the thread-pool internals below `ThreadPoolTaskExecutor`
- `ThreadLocal`, its leak hazards in pooled threads, and `InheritableThreadLocal`
- Concurrent collections in depth — `ConcurrentHashMap` internals, `CopyOnWriteArrayList`, the `BlockingQueue` family
- Coordination — `CountDownLatch`, `CyclicBarrier`, `Semaphore`, `Phaser`
- `ForkJoinPool`, work-stealing, and why parallel streams are a trap
- **Virtual threads & structured concurrency** (the reason this module targets 21)
- Diagnostics — thread dumps, `jcmd`, JFR, false sharing, contention profiling
