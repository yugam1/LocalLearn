...
class Transaction {
    ...
    boolean isFraudulant;
    boolean isDisputed;

    Transaction(String id, String merchId, Double amt, String code, boolean fraud) {
        chargeId = id;
        merchantId = merchId;
        amount = amt;
...
class Merchant {
    ...
    boolean aboveThreshold(int total, int count, int threshold) {
        return merchantCategory.aboveThreshold(count, total) && (total >= threshold);
    }
...
class MerchantCategory {
    ...
    MerchantCategory(String cc, String thres) {
        categoryId = cc;
        Double num = Double.parseDouble(thres);
    ...
    boolean aboveThreshold(int count, int total) {
        if (thresholdType.equals("PERCENT")) {
            return ((double) count * 100 / (double) total) >= threshold;
        } else {
...
class Solution {
    ...
    Map<String, MerchantCategory> merchantCategoryDb;
    ...
    public String detectFraud(
        ...
        Set<String> fraudCodeSet = new HashSet<>(List.of(fraudulentCodes.split(",")));
        merchantDb = new HashMap<>();
        merchantCategoryDb = new HashMap<>();
        transactionDb = new HashMap<>();

        /**
         * [MCC,5814,2, MERCHANT,m1,5814, CHARGE,c1,m1,100,fraud,
         * CHARGE,c2,m1,100,fraud, DISPUTE,c1, DISPUTE,c2]
         */
        ...
        for (String operation : operations) {
            ...
            switch (data[0]) {
                case "MCC":
                    MerchantCategory mc = new MerchantCategory(data[1], data[2]);
                    merchantCategoryDb.put(data[1], mc);
                    break;

                case "MERCHANT":
                    Merchant m = new Merchant(data[1], data[2]);
                    merchantDb.put(data[1], m);
                    MerchantCategory cat = merchantCategoryDb.get(data[2]);
                    m.merchantCategory = cat;
                    break;

                case "CHARGE":
                    Transaction t;
                    if (!transactionDb.containsKey(data[1])) {
                        t = new Transaction(data[1]);
                        transactionDb.put(data[1], t);
                    }
                    t = transactionDb.get(data[1]);
                    // "CHARGE,c1,m1,100,do_not_honor"
                    t.merchantId = (data[2]);
                    t.amount = Double.parseDouble(data[3]);
                    t.transactionCode = (data[4]);
                    t.isFraudulant = fraudCodeSet.contains(data[4]);
                    Merchant curr = merchantDb.get(data[2]);
                    curr.transactions.add(t);
                    break;

                case "DISPUTE":
                    Transaction dt;
                    if (!transactionDb.containsKey(data[1])) {
                        dt = new Transaction(data[1]);
                        transactionDb.put(data[1], dt);
                    }
                    dt = transactionDb.get(data[1]);
                    dt.isDisputed = true;
                    break;
        ...
        for (Merchant m : merchantDb.values()) {
            ...
            for (Transaction t : m.transactions) {
                if (t.isFraudulant && !t.isDisputed) {
                    fraudCount++;
                }
                totalCount++;
                revaluate++;
                if (m.aboveThreshold(totalCount, fraudCount, minTransactions)) {
                    res.add(m.merchantId);
                    revaluate = 0;
                } else if (res.contains(m.merchantId)) {
                    res.remove(m.merchantId);
                }
            }
        }
        ...
        test(
                () -> solution.detectFraud(
                        ...
                        new String[] {
                                ...
                                "CHARGE,c2,m1,100,approved",
                                "CHARGE,c3,m1,100,approved",
                                "CHARGE,c4,m1,100,approved", // m1: 1/4 = 25% → FRAUDULENT
            ...
            test(
                    () -> solution.detectFraud(
                            ...
                            new String[] {
                                    ...
                                    "CHARGE,c1,m1,100,fraud", // Should be treated as disputed
                                    "CHARGE,c2,m1,100,fraud",
                                    "CHARGE,c3,m1,100,fraud"
                            },
                            1),
