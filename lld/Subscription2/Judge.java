...
class User {
        ...
        User(String name, String plan, Integer start, Integer dur) {
                this.name = name;
                this.plan = plan;
                this.startDate = start;
                this.endDate = start + dur;
                this.isActive = true;
        }
...
class Notification implements Comparable<Notification> {
        Integer dateOfNotification;
        String message;
        String userName;
        boolean isActive;

        Notification(Integer date, String mess, User u) {
        ...
        public int compareTo(Notification sec) {
                if (this.dateOfNotification == sec.dateOfNotification) {
                        return this.message.compareTo(sec.message);
                }
                return Integer.compare(this.dateOfNotification, sec.dateOfNotification);
        }
...
class NotificationScheduler {
        ...
        NotificationScheduler(Map<String, String> sendSchedule) {
                this.notificationsDb = new ArrayList<>();
                this.rules = sendSchedule;
        }

        void scheduleAllNotificationsForUser(User u, Integer schuduleDate) {
                for (var rule : this.rules.entrySet()) {
                        String prefix = rule.getValue();
                        Integer date = 0;
                        if (rule.getKey().equals("start")) {
                                date = u.startDate;
                        } else if (rule.getKey().equals("end")) {
                                date = u.endDate;
                        } else {
                                date = u.endDate + Integer.parseInt(rule.getKey());
                                if (date < u.startDate)
                                        continue;

                        }
                        if (date < schuduleDate)
                                continue;
                        String message = String.format("%d: [%s] Subscription for %s (%s)", date, prefix, u.name,
                                        u.plan);

                        scheduleNotifications(date, message, u);

                }
        }

        void scheduleNotifications(Integer date, String message, User u) {
                notificationsDb.add(new Notification(date, message, u));
        }

        void cancelNotification(User u, Integer date) {
                for (Notification noti : notificationsDb) {
                        if (noti.userName.equals(u.name) && noti.dateOfNotification > date) {
                                noti.isActive = false;
                        }
                }
        }

        void schedulePlanChangeNotification(User u, Integer date, String prevPlan) {
                cancelNotification(u, date);
                String plan = prevPlan + " -> " + u.plan;

                String message = String.format("%d: [Changed] Subscription for %s (%s)", date, u.name,
                                plan);

                notificationsDb.add(new Notification(date, message, u));

                scheduleAllNotificationsForUser(u, date);

        }

        List<String> getScheduledNotification() {
                return notificationsDb.stream().filter(n -> n.isActive).sorted().map(n1 -> n1.message).toList();
        }
}
...
class Solution {
        ...
        public List<String> scheduleNotifications(
                ...
                NotificationScheduler scheduler = new NotificationScheduler(sendSchedule);
                usersDb = new HashMap<>();

                for (var user : userAccounts) {
                        User u = new User((String) user.get("name"), (String) user.get("plan"),
                                        (Integer) user.get("account_date"), (Integer) user.get("duration"));
                        scheduler.scheduleAllNotificationsForUser(u, 0);
                        usersDb.put(u.name, u);
                }

                for (var change : changes) {
                        String userName = (String) change.get("name");
                        String nPlan = (String) change.get("new_plan");
                        Integer changeDate = (Integer) change.get("change_date");
                        Integer extension = (Integer) change.get("extension");
                        User u = usersDb.get(userName);
                        if (u == null)
                                continue;

                        if (nPlan != null) {
                                String prevPlan = u.plan;
                                u.plan = nPlan;
                                scheduler.schedulePlanChangeNotification(u, changeDate, prevPlan);
                        } else {
                                scheduler.cancelNotification(u, changeDate);
                                u.endDate = u.endDate + extension;
                                // 20: [Renewed] Subscription for Bob (Pro)
                                scheduler.scheduleNotifications(changeDate,
                                                String.format("%d: [Renewed] Subscription for %s (%s)", changeDate,
                                                                u.name,
                                                                u.plan),
                                                u);
                                scheduler.scheduleAllNotificationsForUser(u, changeDate);
                        }
                ...
                return scheduler.getScheduledNotification();
        }
}
        ...
        private static void runPart3Tests(Solution solution) {
                ...
                test(
                                ...
                                Arrays.asList(
                                                ...
                                                "16: [Upcoming Expiry] Subscription for Charlie (Pro)",
